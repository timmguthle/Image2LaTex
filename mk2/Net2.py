...
class Encoder_CNN(keras.layers.Layer):
    ...
    def __init__(self, **kwargs):
        ...
        self.cnn3 = keras.layers.Conv2D(512, (3,3), activation='relu', padding='same', name='cnn3')
...
class Row_Encoder(keras.layers.Layer):
    ...
    def __init__(self, units=256, **kwargs):
        ...
        self.bi = keras.layers.Bidirectional(self.gru, name='bi1', merge_mode='concat')
    ...
    def build(self, input_shape):
        if input_shape[0] == None:
            self.batch_size_known = False
            return 
            
        # initialize the the initial states as weights for the RNN
        self.w1 = self.add_weight(shape=(input_shape[0], self.units), initializer='glorot_uniform' , trainable=True, name='w1')
        self.w2 = self.add_weight(shape=(input_shape[0], self.units), initializer='glorot_uniform' , trainable=True, name='w2')
        # self.w3 = self.add_weight(shape=(input_shape[0], 256), initializer='zeros', trainable=True)
        # self.w4 = self.add_weight(shape=(input_shape[0], 256), initializer='zeros', trainable=True)
        self.initial_state = [self.w1, self.w2]
...
class All_Row_Encoder(keras.layers.Layer):
    ...
    def __init__(self, units=256, **kwargs):
        ...
        self.units = units
        self.all_row_units = units * 2 # for concat of the two directions of the bidirectional RNN

    @tf.function
    def call(self, x):
        first_pass = True
        new_tensor = tf.zeros((1,1,512))
        while tf.size(x) != 0:
            # initialize the tensorflow loop to generate a graph
            tf.autograph.experimental.set_loop_options(shape_invariants=((new_tensor, tf.TensorShape([None, None, 512])), (x, tf.TensorShape([None, None, None, 512]))))
...
class My_Attention(keras.layers.Layer):
    def __init__(self, **kwargs):
        ...
        self.mha = keras.layers.MultiHeadAttention(num_heads=1, key_dim=16, name='mha1')
...
class Decoder(keras.layers.Layer):
    ...
    def __init__(self, text_processer, **kwargs):
        super().__init__(**kwargs)
...
class Decompiler(keras.Model):
    ...
    def __init__(self, text_processer, drop_out_value=0.5, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.CNN_Encoder = Encoder_CNN(name='encoder')
        self.Row_Encoder = All_Row_Encoder(name='row_encoder')
        self.Decoder = Decoder(text_processer, name='my_decoder')
