...
class Encoder_CNN(keras.layers.Layer):
    ...
    Output: (batch_size, h, w, self.cnn_out_channels)
    '''
...
class Row_Encoder(keras.layers.Layer):
    ...
    def build(self, input_shape):
        '''
        fix: instead of initializing the weights with the batch size, I have to just use one in the first dimension 
        and replicate it to the size of the batch.
        '''

        # initialize the the initial states as weights for the RNN. Only two state per direction and one per nr. units 
        self.w1 = self.add_weight(shape=(1, self.units), initializer='glorot_uniform' , trainable=True, name='w1')
        self.w2 = self.add_weight(shape=(1, self.units), initializer='glorot_uniform' , trainable=True, name='w2')
        self.w3 = self.add_weight(shape=(1, self.units), trainable=True,name='w3')
        self.w4 = self.add_weight(shape=(1, self.units), trainable=True, name='w4')
    ...
    def call(self, inputs):
        if inputs.shape[0] is not None:
            ...
            self.initial_state = [tf.tile(self.w1, [self.batch_size, 1]), tf.tile(self.w2, [self.batch_size, 1]), tf.tile(self.w3, [self.batch_size, 1]), tf.tile(self.w4, [self.batch_size, 1])]
...
class All_Row_Encoder(keras.layers.Layer):
    '''
    Input shape (None, h, w, self.all_row_units)
    Output shape (None, (h*w), self.all_row_units)
...
class My_Attention(keras.layers.Layer):
    ...
    def call(self, x, context):
        ...
        attention_scores shape: (batch, token_length, (h*w))

        '''
        output, scores = self.mha(query=x, value=context, return_attention_scores=True) # type: ignore
        self.attention_scores = tf.reduce_mean(scores, axis=1) # save the scores for plotting
...
class Decoder(keras.layers.Layer):
    ...
    def __init__(self, text_processer, dec_units=512, **kwargs):
        ...
        self.gru = keras.layers.GRU(self.dec_units, return_sequences=True, return_state=True, name='gru2')
    ...
    def call(self, context, x, state=None, return_state=False):
        ...
        self.attention_scores = self.attention.attention_scores # save the scores for plotting
    ...
    def tokens_to_text(self, input):
        '''
        called at the end of the decompile loop to convert the tokens to a string
        '''
        text = tf.strings.reduce_join(self.id_to_word(input), axis=-1, separator=' ')
        return text.numpy()[0].decode('utf-8') # return type is string
    
    def label_to_text(self, input):
        text = tf.strings.reduce_join(self.id_to_word(input), axis=-1, separator=' ')
        text = text.numpy().decode() # return type is string
...
class Decompiler(keras.Model):
    ...
    def call(self, inputs):
        ...
        # for debugging purposes leave out the Row_Encoder and use simple concat layer instead
    ...
    def decompile(self, image, max_len=100):
        ...
        self.context_shape = context.shape # type: ignore
        ...
        for _ in range(max_len):
            ...
            tokens.append(token)
            attention_scores.append(self.Decoder.attention_scores)


        tokens = tf.concat(tokens, axis=-1)
        self.attention_scores = tf.concat(attention_scores, axis=1) # to shape (batch, token_length, (h*w))

        result = self.Decoder.tokens_to_text(tokens)
        return result
    
    def plot_attention(self, image):
        '''
        Generates a plot of the attention scores overlayed over the equation for each token. 
        Input: 
            image with shape: (1, h, w, 1)

        returns:
            predicted formula as a string
        '''

        # pass in one image and plot the attention scores
        pred = self.decompile(image)
        assert type(pred) == str
        print(self.attention_scores.shape)
        scores = self.attention_scores[0] # (1, token_length, (h*w)) -> (token_length, (h*w))
        token_length = scores.shape[0]
        h, w = self.context_shape[1], self.context_shape[2]
        assert (h * w) == scores.shape[1]
        

        for i in range(token_length):
            attention_map = scores[i].numpy().reshape(h,w)
            attention_map = tf.expand_dims(attention_map, axis=-1)
            attention_map = tf.image.resize(attention_map, (image.shape[1], image.shape[2]))
            fig, ax = plt.subplots(1, 1)
            
            ax.axis('off')
            ax.imshow(image[0,:,:,0], cmap='gray')
            ax.imshow(attention_map, cmap='viridis', alpha=0.4)
            
            print(pred.split(' ')[max(0,i-8):i])

            # break the loop if mask token is reached
            if pred.split(' ')[i] == '':
                break
...
if __name__ == '__main__':
    ...
    # debugging stuff
    input = keras.layers.Input(shape=(60, 512))
    x = Row_Encoder()(input)
    Model = keras.Model(inputs=input, outputs=x)
    Model.summary()

    test_input = tf.random.normal((1, 60, 512))
